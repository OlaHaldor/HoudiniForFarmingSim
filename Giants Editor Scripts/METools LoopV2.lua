...
function RDM_createField(centerPoint, fieldsTG, template, newField)
    ...
    if newField then
        ...
        field = getChildAt(fieldsTG, numberOfFields-1)
        fieldDimensions = getChildAt(field, 0)
    end
    ...
    if newField then
        ...
        print("added to existing field dimensions and expanded Field '"..getName(field).."'")
    end
end
...
local shapesTG = getSelection(0)
local fieldsTG = getSelection(1)
local template = getSelection(2)
local prevName = ""
local newField = true

local numberOfShapes = getNumOfChildren(shapesTG)

for i = 0, numberOfShapes-1 do   
    local centerPoint = getChildAt(shapesTG, i)
    if prevName ~= getName(centerPoint) then
        prevName = getName(centerPoint)
        newField = true
    else
        newField = false
    end
    RDM_createField(centerPoint, fieldsTG, template, newField)
end
